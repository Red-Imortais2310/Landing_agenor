...
with sync_playwright() as p:
    ...
    for i, link in enumerate(LINKS):
        print(f'[{i+1}/{len(LINKS)}] Acessando: {link}')
        try:
            page.goto(link)
            page.wait_for_timeout(3000)
            
            # Tenta pegar o nome do curriculo
            nome_elem = page.query_selector('h2.nome, .nome-completo, h1')
            if nome_elem:
                nome = nome_elem.inner_text().strip()
            else:
                # Pega do titulo da pagina
                nome = page.title().replace('Currículo Lattes', '').replace('- CNPq', '').strip()
            ...
            print(f'  ERRO: {e}')
            resultados.append({'nome': 'ERRO', 'link': link})
...
# Salva resultado
with open('lattes_nomeado.txt', 'w', encoding='utf-8') as f:
    f.write('MAPEAMENTO LATTES - IFSP CUBATÃO\n')
    f.write('=' * 60 + '\n\n')
    for r in resultados:
        f.write(f"{r['nome']} | {r['link']}\n")

print('\n\nSALVO EM: lattes_nomeado.txt')
